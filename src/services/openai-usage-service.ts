...
/**
 * OpenAI ChatGPT Usage API endpoint
 * Reference: https://chatgpt.com/backend-api/wham/usage
 */
const OPENAI_USAGE_API_URL = 'https://chatgpt.com/backend-api/wham/usage';
...
export async function fetchOpenAIUsage(): Promise<OpenAIUsageData> {
  try {
    ...
    const response = await simpleFetch(OPENAI_USAGE_API_URL, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      logger.error('[OpenAIUsage] API error:', response.status, errorText);
      throw new Error(`OpenAI Usage API error: ${response.status} - ${errorText}`);
    }

    const apiData = (await response.json()) as OpenAIApiResponse;
    ...
    const data: OpenAIUsageData = {
      five_hour: transformWindow(apiData.rate_limit.primary_window),
      seven_day: transformWindow(apiData.rate_limit.secondary_window),
    ...
    if (apiData.credits) {
      data.credits = {
        balance: Number.parseFloat(apiData.credits.balance) || 0,
        has_credits: apiData.credits.has_credits,
        unlimited: apiData.credits.unlimited,
      };
    }

    // Add code review utilization if present
    if (apiData.code_review_rate_limit?.primary_window) {
      data.code_review_utilization = apiData.code_review_rate_limit.primary_window.used_percent;
    }
